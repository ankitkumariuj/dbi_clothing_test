  ...
  $.ajax({
    ...
    success: function (response) {
      ...
      if (response.status !== false && Array.isArray(response)) {
        ...
        const kids = response.filter((item) => item.gender === "kids");
        console.log( "ftech category kids" + kids);
  ...
  $.ajax({
    ...
    success: function (response) {
      ...
      if (response.status !== false && Array.isArray(response)) {
        ...
        // ✅ Limit to only first 20 categories
        const categories = response;
        const women_categories = response.filter((item) => item.gender === "kids")
  ...
  await $.ajax({
    ...
    success: function (response) {
      if (response.status !== false) {
        ...
        const kids_banner = response.filter((item) => item.gender === "kids");

        console.log("women all banner", kids_banner);


        let mainData = kids_banner.filter((item) => item.banner_type === "main");
        let upperData = kids_banner.filter((item) => item.banner_type === "upper");
        let lowerData = kids_banner.filter((item) => item.banner_type === "lower");
...
console.log("main_banner data kids" + mainData)
          bannerData += `
                    <div class="item">
                        <div class="card">
...
async function GetSubCategory(subId) {
  ...
  if (res.length > 0) {
    ...
    const subcate_kids = res.filter((item) => item.gender === "kids");

    subcate_kids.forEach((item, index) => {
...
const fetchTodayBestDeal = () => {
  ...
  $.ajax({
    ...
    success: function (response) {
      ...
      const trendwomen = response.filter((item)=> item.gender === "kids");
...
const fetchprodaccordtotittle = () => {
  ...
  $.ajax({
    ...
    success: function (response) {
      ...
      if (response && response.length > 0) {
        ...
        const women_title1 = response.filter((item) => item.gender === "kids");
 ...
 <div class="product">
            <a class="product-img" href="singlep.html?pid=${item.id}" onclick="saveProductName('${item.name}')">
              ...
              <span class="old-pricepcs">MRP ₹${parseInt(item.mrp)} | Stock: ${item.stock}Pcs</span>
  ...
  $.ajax({
    ...
    success: function (response) {
      ...
      if (response && response.length > 0) {
        ...
        const women_title2 = response.filter((item) => item.gender === "kids");
  ...
  $.ajax({
    ...
    success: function (response) {
      ...
      if (response && response.length > 0) {
        ...
        let html = "";
        const women_title3 = response.filter((item) => item.gender === "kids");
...
const mendata= data.filter((item)=> item.gender === "kids");
...
const fetchTrendImages = () => {
    $.ajax({
        url: API_URL,
        type: "POST",
        dataType: "json",
        data: { type: "fetchtrendbanner" },
        success: function (res) {
            ...
            if (res.status == 1) {
                ...
                res.data.forEach((item) => {
                    ...
                    if (item.trend_type !== "kids") return;
                    ...
                    images.forEach(img => {
                        html += `
                            <img 
                                src="${image_url}trend_banners/${img}" 
                ...
                if (html === "") {
                   $(".shopby_trend").hide();
                   $(".shopby_trend_title").hide();
                }

                $(".shopby_trend").html(html);

            } else {
                $(".shopby_trend").html("<p>No images found</p>");
            }
