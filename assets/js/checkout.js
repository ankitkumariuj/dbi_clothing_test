...
$(document).ready(function(){
    let savedValue = localStorage.getItem("paymentMethod");
    if(savedValue){
        $('input[name="payment"][value="'+savedValue+'"]').prop('checked', true);
...
const placeOrder = () => {
    ...
    let paymentType  = localStorage.getItem("paymentMethod"); 
...
,
    ...
    localStorage.removeItem("final_price", finalAmount);
...
function applyCoupon(code, discountType, discountValue, minAmount, couponId) {
    ...
    let originalTotal = parseFloat(localStorage.getItem("main_price"));
    let cartTotal;

    
    if (!originalTotal) {
        originalTotal = parseFloat(localStorage.getItem("total_price")) || 0;
        localStorage.setItem("main_price", originalTotal);
    }
    ...
    $("#discount").text("- ₹" + discount.toFixed(2)).css("color", "green");
    ...
    localStorage.setItem("total_price", finalAmount);
