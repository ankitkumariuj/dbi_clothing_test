...
function toggleSort(radio, type, action = null) {
...
const cat_filter = (type, action) => {
  // Agar same filter dubara click hua
  if (lastFilter.type === type && lastFilter.action === action) {
    renderProduct(allProducts); // normal state
    lastFilter = { type: null, action: null };
    return;
  }
  ...
  if (type === "cat_name") {
    data = currentData.filter((a) => a.id == action);
  }

  if (type === "size") {
    data = currentData.filter((a) => a.size == action);
  }

  if (type === "under") {
    data = currentData.filter((a) => a.selling_price < parseInt(action));
  }
  ...
  if (type === "above") {
    data = currentData.filter((a) => a.selling_price > 1000);
  }
  ...
  if (type === "color") {
    data = currentData.filter((a) => a.color == action);
  }
...
const addProductRating = (total_rating, ratingCount, pid) => {
  if (total_rating != 0 && ratingCount != 0) {
    ...
    ratingsStarCount = Math.round(ratingsStarCount); 
...
const fetchVariantProductByAction = async (action, value, id) => {
  ...
  if (
    lastVariantFilter.action === action &&
    JSON.stringify(lastVariantFilter.value) === JSON.stringify(value)
  ) {
    // reset the filter
    $.ajax({
      url: API_URL,
    ...
    lastVariantFilter = { action: null, value: null };
    return;
  }
 ...
 // load variants product
  $.ajax({
    ...
    success: function (response) {
      ...
      renderVarProduct(response);

      //  save last filter
      lastVariantFilter = { action, value };
    },
  });
};
  ...
  if (data.status !== false) {
    data.map((item, index) => {
      ...
      productHtml += `
             ...
             <div class="product">
            <a class="product-img" href="singlep.html?pid=${item.product_id}" onclick="saveProductName('${item.name}')">
              <img src="${
              ...
              <span class="old-pricepcs">MRP ₹${parseInt(item.mrp)} | Stock: ${item.qty}Pcs</span>
              <div class="discount-label">${Math.round(((item.mrp - item.selling_price) / item.mrp) * 100)}% Margin
  </div>
               <!-- Heart Icon with Label -->
