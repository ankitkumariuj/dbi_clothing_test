...
Object.values(mergedItems).forEach(item => {
     const price = item.variant_selling_price
...
totalAmount += itemTotal;
      ...
      itemsHtml += `
        ...
        <h4>Item Size:  ${item.item_size}</h4>
        <h3>Item Quantity : ${order.quantityno}</h3>
  <img 
  src="${item.variant_single_image
        ? `${image_url}variant/main/${item.variant_single_image}`
  ...
  <div class="status-box ${statusClass}">
    ...
    <div class="summary-list">
        ...
        <div class="summary-row">
            ...
            <div class="item-prices" style="color: green"> - ₹${order.discount_value}</div>
        </div>
 ...
 <div class="upload-container">
    ...
    <input type="file" id="fileInput" name="image_1" accept="image/*" hidden onchange="previewImage(event)">
        ...
        <div class="give-star-submit">
          <button class="give-star-submit-btn" onclick="addRating(${order.product_id})">
            <h2>Submit Review</h2>
          </button>
        </div>
      </div>
...
console.log("Filtered order items:", orderItems);
      ...
      orderItems.forEach((item, index) => {
        const q = item.nop || item.quantity || 1;
        const u = item.unit || "";
        const mrp = parseFloat(item.mrp || 0);
        const sell = parseFloat(item.variant_selling_price ? item.variant_selling_price : item.product_selling_price || 0);
        const discount_values = parseInt(item.discount_value || 0);
        const subtotal = parseInt(item.total_amount || 0);
        const total = q * sell;
        totalPrice += total;
        totalMrp += q * sell;
main_price+= subtotal;
...
console.log("discount first" + discount);
        ...
        html += `
          <tr>
            <td>${q} ${u}</td>
            <td>${item.product_name || "Unnamed"}<br>
            Unit Price: ₹${sell}<br>
            Mrp Price: ₹${item.variant_mrp ? item.variant_mrp : item.product_mrp}
      ...
      $(".billtotalDiscount").html(format(discount));
      $(".billsubTotal").html(format(totalPrice - discount));
      $(".handlingCharge").html(format(orderData.handling_charge));
      $(".billdelCharege").html(format(orderData.del_charge));
      $(".billgrandTotal").html(format(totalPrice - discount));
