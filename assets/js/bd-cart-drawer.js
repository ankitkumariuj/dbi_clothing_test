...
<p class="color-name" data-color="${isVariant ? item.variant_color : item.product_color}">
 ${isVariant ? item.variant_color_name : item.color_name}
</p>
            </div>
...
<div class="cart-quantity-input-box">
 ...
 $(".color-name").each(async function () {
  let colorCode = $(this).data("color");
  let colorName = await getColorName(colorCode);
