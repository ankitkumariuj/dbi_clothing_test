...
const localStorageCacheKey = "colorNameCache";


try {
    const storedCache = JSON.parse(localStorage.getItem(localStorageCacheKey));
    if (storedCache) Object.assign(colorNameCache, storedCache);
} catch (e) {}
...
async function getColorName(hex) {
    ...
    if (colorNameCache[hex]) return colorNameCache[hex];

    try {
        const res = await fetch(`https://www.thecolorapi.com/id?hex=${hex}`);
        const data = await res.json();
        const name = data?.name?.value || `#${hex}`;

        colorNameCache[hex] = name;
        localStorage.setItem(localStorageCacheKey, JSON.stringify(colorNameCache));
...
async function populateColorNames() {
    const colorElements = $(".color-name");
    const colorsToFetch = [];

    colorElements.each(function() {
        const colorCode = $(this).data("color");
        if (colorCode && !colorNameCache[colorCode.replace("#", "").trim()]) {
            colorsToFetch.push(colorCode);
        }
    });

    const uniqueColors = [...new Set(colorsToFetch)];

    await Promise.all(uniqueColors.map(color => getColorName(color)));

  
    colorElements.each(function() {
        const colorCode = $(this).data("color");
        $(this).text(colorNameCache[colorCode.replace("#", "").trim()] || colorCode);
    });
}


  // push main product as variant
...
if (allVariants.length === 0) {
  $(".color_container, .selector-title").hide();
} else {
    ...
    for (const v of allVariants) {
      ...
      if (v.color && !uniqueColors.has(v.color)) {
        ...
        let imageSource = v.single_image
          ? (v.isMainProduct
              ? "product/main/" + v.single_image
              : "variant/main/" + v.single_image)
          : null;

        
        let colorName = v.color_name;
        if (!colorName) {
          colorName = await getColorName(v.color);
        }
        ...
        variantHTML += `
          <div class="color_main">
            <div class="color-item"
                 data-color="${v.color}"
                 id="variant-${v.id}"
                 onclick="loadvariant(globalResponse, ${v.id})">

              <div class="color-box">
                ${
                  imageSource
                    ? `<img src="${image_url + imageSource}" alt="Variant Image">`
...
<path d="M11.4861 2.56665C13.3747 2.56671 14.8708 4.02899 14.8708 5.79321C14.8708 7.95177 13.8257 9.68625 12.5037 10.9788C11.1765 12.2761 9.60564 13.0903 8.66187 13.3977L8.65796 13.3997C8.60686 13.4169 8.5068 13.4329 8.38354 13.4329C8.26044 13.4328 8.16116 13.4169 8.11011 13.3997L8.10522 13.3977C7.16133 13.0902 5.59051 12.2762 4.26343 10.9788C2.94139 9.68625 1.89722 7.95177 1.89722 5.79321C1.89728 4.02895 3.39329 2.56665 5.28198 2.56665C6.39734 2.56671 7.37695 3.08101 7.98901 3.86743C8.08368 3.98901 8.22946 4.05971 8.38354 4.05981C8.53761 4.05981 8.68334 3.9889 8.77808 3.86743C9.38988 3.08135 10.3772 2.56665 11.4861 2.56665Z" stroke="#FD3E75" stroke-linecap="round" stroke-linejoin="round"/>
                ...
                <div id="svg" onclick="shareProduct()">
...
<path opacity="0.01" fill-rule="evenodd" clip-rule="evenodd" d="M0.500067 16.5L0.500067 0.5L17.2681 0.5L17.2681 16.5L0.500067 16.5Z" stroke="#272727"/>
        ...
        <div class="product-detail">
            ...
            <div class="price-row">
                ...
                <div class="qty-box">Stock: ${data.stock} Pcs</div>
            </div>
<div class="coupon-container">
            ...
            <div class="selector-section">
                <div class="size-header">
                    ...
                    <div class="size-chart-link"  onclick="show_charts()">Size Chart <i class="fas fa-chevron-right"></i></div>
                </div>
            ...
            <div class="product">
                ...
                <div class="product_info" style="padding-inline: 3%">
                    ...
                    <div class="price-container">
                        ...
                        <span class="old-pricepcs">MRP ₹${item.mrp} | Stock: ${item.stock}Pcs</span>
...
function fetchcoupon() {
    $.ajax({
        ...
        <div class="margin-badge">
            Upto ₹${coupon.discount_value} off
        </div>
    </div>
